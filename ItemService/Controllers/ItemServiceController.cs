...
/// 
/// Generic controller for all Item Service entities
...
namespace ItemService.Controllers {
    // Limit generic type T to be a class and a IBase model. Allows for use of DbContext.Set() (Type needs to be class) and to only allow Service Model Types
    public abstract class ItemServiceController<T> : ControllerBase, IApiController<T> where T : class, IBaseModel {
        // ItemContext access set to protected so child classes can utilize it
        protected readonly ItemContext _context;

        // Readonly param that gets ItemContexxt Iqueryable based on passed generic type T
        private IQueryable<T> GetDynamicQueryable {
            get => _context.Set<T>();
        }

        // Sets maximum number of entities for writing with any item service api
        ...
        /// <summary>
        /// Constructor for a Item Service controller generically
        /// 
        /// Checks to see if generic Type T pertains to a valid DB set in type Item Context
        /// </summary>
        /// <param name="context">Item Context for database operations</param>
        public ItemServiceController(ItemContext context){
            if(context.Set<T>() == null){
                throw new Exception("Type of T (" + typeof(T).ToString() + ") Not DbSet of ItemContext.");
            }
            _context = context;
        }
